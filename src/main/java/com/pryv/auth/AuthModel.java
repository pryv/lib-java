...
public class AuthModel {
	...
	public AuthModel(AuthController pController, String requestingAppId, List<Permission> permissions, String language,
			String returnURL) {
	...
	public void startLogin() {
		try {
			...
			Request.Post(Pryv.REGISTRATION_URL).bodyString(jsonRequest, ContentType.APPLICATION_JSON).execute()
					.handleResponse(signInResponseHandler);
	...
	private ResponseHandler<String> signInResponseHandler = new ResponseHandler<String>() {
		...
		/**
		 * unique class method that retrieves HttpResponse's components and
		 * calls the appropriate controller's methods
		 */
		@Override
